...
import { computeAccuracyInputs, getShotgunMultipler } from "./Simulator.js";
...
function getShotTypes(damageModel) {
  ...
  if (!ammo || !weapon.ammo) {
    // ie, vanilla-style compatibleAmmo
...
function getAttemptsPerTurn(ruleset, state, key = "weapon") {
  ...
  const soldierEntry = entries[soldier].soldiers ?? entries[soldier].units; //if not a soldier, it's a vanilla hwp
  ...
  actionTypes.forEach((type) => {
    ...
    // TODO: Account for reload time
    if (tuCost) {
      result[type] = flatCost
        ? Math.floor(tu / tuCost)
        : Math.floor(100 / tuCost);
    }
  });
  return result;
}
...
export function loadDependencies() {
  ...
  // load WASM code
  const wasm = loadWASM().then((iface) => {
    getTTK = iface.getTTK;
    window.getTTK = getTTK;
  });
...
function lookupAcc(accuracyData, targetWidth, targetHeight, acc, distance) {
  const offset =
    widthOffsets[targetWidth] * 132000 + // 50 * 110 * 24
...
export function getChartData(ruleset, state) {
  ...
  const compareDamageModel =
    state.compare &&
    new DamageSimulator(ruleset, state, "compareWeapon", "compareAmmo");
  const compareShotTypes = state.compare && getShotTypes(compareDamageModel);
  const compareAttemptsPerTurnByType =
    state.compare && getAttemptsPerTurn(ruleset, state, "compareWeapon");

  const data = [];

  const shotgunMultiplier = getShotgunMultipler(ruleset, state); // will just return 1 for non-shotgun weapons
  const compareShotgunMultiplier = getShotgunMultipler(
    ruleset,
    state,
    "compareWeapon",
    "compareAmmo"
  ); // will just return 1 for non-shotgun weapons

  for (let distance = 1; distance <= 50; distance++) {
    const dataPoint = { distance };
    for (let shotType of shotTypes) {
      const accuracyParameters = computeAccuracyInputs(
        ruleset,
        shotType,
        distance,
        state
      ...
      const hitRatio = singleHitAccuracy * shotCount * shotgunMultiplier;
      ...
      dataPoint[`${shotType}Damage`] =
        ((rangeModifiedDamage * hitRatio) / 100) * attemptsPerTurn;
      // function getTTK(health: i32, armor: i32, rolls: i32, lowLimit: i32, highLimit: i32, dmg: i32, hitChance: f64) : f64[] {
      console.log(
        `${shotType}TTK_${distance}`,
        ...damageModel.TTKParams,
        singleHitAccuracy / 100
      );
      const ttk = getTTK(
        Math.floor(shotCount * shotgunMultiplier), //HACK: ttk doesn't handle partial hits, so just round down.
        ...damageModel.TTKParams,
        damageModel.getTTKPower(distance),
        singleHitAccuracy / 100
      );
    ...
    if (state.compare) {
      for (let shotType of compareShotTypes) {
        ...
        const singleHitAccuracy = lookupAcc(accuracyData, ...accuracyInputs);
        const hitRatio =
          singleHitAccuracy * shotCount * compareShotgunMultiplier;
        const compareAttemptsPerTurn = compareAttemptsPerTurnByType[shotType];
        const rangeModifiedDamage =
          compareDamageModel.getRangeModifiedDamage(distance);

        dataPoint[`Compare${shotType}HitRatio`] = hitRatio;
        dataPoint[`Compare${shotType}Damage`] =
          ((rangeModifiedDamage * hitRatio) / 100) * compareAttemptsPerTurn;
        const ttk = getTTK(
          Math.floor(shotCount * compareShotgunMultiplier), //HACK: ttk doesn't handle partial hits, so just round down.
          ...compareDamageModel.TTKParams,
          compareDamageModel.getTTKPower(distance),
          singleHitAccuracy / 100
        );
  ...
  return {
    data,
    weaponEntry: damageModel.weaponEntry,
    compareWeaponEntry: compareDamageModel?.weaponEntry,
  };
}
...
window.getChartData = function (
  ...
  const result = getChartData(window.db.ruleset, {
    stat: statMode,
    direction,
    soldier,
    armor,
